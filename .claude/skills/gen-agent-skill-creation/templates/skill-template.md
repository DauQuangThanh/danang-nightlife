---
name: skill-name
description: "[Active Verbs]. Use when [Specific Scenarios] or when user mentions [Keywords]."
license: Internal use, an Fpt Software Intellectual Property
metadata:
  author: Your Name
  version: "1.0"
  last_updated: "YYYY-MM-DD"
---

# Skill Name

## Overview

[Brief 2-3 sentence explanation of what this skill does and its primary purpose]

## When to Use

- [Specific scenario 1]
- [Specific scenario 2]
- [Specific scenario 3]
- [User request pattern]

## Prerequisites

**Required:**

- [Tool/package name] version X.X+
- [Another requirement]

**Optional:**

- [Optional tool/package]

## Instructions

### Step 1: [Action Name]

[Clear, specific instructions for this step]

**Details:**

- [Specific sub-step or consideration]
- [Another important point]

**Example:**

```bash
command --option value
```

### Step 2: [Action Name]

[More instructions]

**Validation:**

- [ ] Check 1
- [ ] Check 2

### Step 3: [Action Name]

[Continue with remaining steps]

## Examples

### Example 1: [Use Case Name]

**Scenario:** [Description of the situation]

**Input:**

```
[Sample input]
```

**Command/Action:**

```bash
command input.txt --output result.txt
```

**Output:**

```
[Expected output]
```

**Explanation:** [What happened and why]

### Example 2: [Another Use Case]

[Similar structure]

## Edge Cases

### Case 1: [Specific Situation]

**Scenario:** [Describe the unusual situation]

**Handling:**

1. [Step to detect/handle]
2. [What to do]
3. [Expected outcome]

**Example:**

```
[Code or command showing handling]
```

### Case 2: [Another Edge Case]

[Similar structure]

## Error Handling

### Error: [Error Type or Message]

**Symptom:**

```
[Error message or behavior]
```

**Cause:** [Why this error occurs]

**Solution:**

1. [Step 1 to fix]
2. [Step 2 to fix]

**Prevention:** [How to avoid this error]

### Error: [Another Error]

[Similar structure]

## Scripts (If Applicable)

### Script Name

**Purpose:** [What the script does]

**Usage (choose your platform):**

```bash
# Bash (macOS/Linux/WSL)
bash scripts/script-name.sh <input> [--option value]

# PowerShell (Windows)
.\scripts\script-name.ps1 -Input <input> [-Option value]
```

**Options:**

- `input` (required): [Description]
- `--option` (optional): [Description]

**Example:**

```bash
# Bash (macOS/Linux/WSL)
bash scripts/process.sh data.csv --output results/

# PowerShell (Windows)
.\scripts\process.ps1 -Input data.csv -Output results/
```

## Quality Checklist

Before considering this task complete:

- [ ] [Requirement 1]
- [ ] [Requirement 2]
- [ ] [Validation check]
- [ ] [Test passed]

## Tips

1. **[Tip Category]**: [Helpful advice]
2. **[Another Tip]**: [More guidance]
3. **[Best Practice]**: [Recommended approach]

## Additional Resources

For more detailed information:

- [references/topic.md](references/topic.md) - [Description]
- [templates/template.ext](templates/template.ext) - [Description]

---

**Note:** [Any important notes or warnings]
