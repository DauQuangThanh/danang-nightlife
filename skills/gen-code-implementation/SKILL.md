...
- **Before coding**: Use `gen-technical-detailed-design` and `gen-coding-plan` skills for planning
- **After coding**: Use `gen-code-review` skill for quality validation
- **Commits**: Use `gen-git-commit` skill to generate semantic commit messages following conventional commit standards
...
```bash
bash <SKILL_DIR>/scripts/check-implementation.sh /path/to/feature --json
```

**Windows PowerShell:**

```powershell
.\<SKILL_DIR>\scripts\check-implementation.ps1 -FeatureDirectory /path/to/feature -Json
```
...
1. **Run `gen-code-review` skill** to validate code quality, error handling, test coverage, and security
2. **Address findings** from review before creating PR/MR
3. **Run `gen-security-review` skill** for security vulnerability checks
...
Cross-platform scripts for checking implementation prerequisites:

```bash
bash <SKILL_DIR>/scripts/check-implementation.sh [feature-dir] [--json]       # macOS/Linux/WSL
.\<SKILL_DIR>\scripts\check-implementation.ps1 [-FeatureDirectory dir] [-Json] # Windows PowerShell
```
...
4. **Commit Frequently**: Commit after each logical unit of work; use `gen-git-commit` skill for semantic commit messages
