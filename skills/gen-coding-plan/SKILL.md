...
- **Tools**: Bash 4.0+ (macOS/Linux/WSL) or PowerShell 5.1+ (Windows) for prerequisite checks
...
```bash
bash <SKILL_DIR>/scripts/check-prerequisites.sh --json
```

**Windows PowerShell:**

```powershell
.\<SKILL_DIR>\scripts\check-prerequisites.ps1 -Json
```
...
```bash
bash <SKILL_DIR>/scripts/check-prerequisites.sh [--json] [--feature-dir /path]    # macOS/Linux/WSL
.\<SKILL_DIR>\scripts\check-prerequisites.ps1 [-Json] [-FeatureDir /path]         # Windows PowerShell
```
...
- Use `gen-code-implementation` skill to execute the implementation tasks
- Use `gen-project-consistency-analysis` skill to validate consistency across all project documents and ensure the task breakdown aligns with architecture, specifications, and design decisions
- Use `gen-code-review` skill after implementation to validate code quality

**Before creating tasks:**

- Ensure `gen-requirement-development` skill has been used to create feature specs
- Ensure `gen-technical-detailed-design` skill has been used to create implementation design
- Consider `gen-architecture-design` skill for product-level architecture (if not yet created)

## Related Resources

- `gen-requirement-development` skill - Create initial feature specifications
- `gen-technical-detailed-design` skill - Create detailed implementation designs
- `gen-project-consistency-analysis` skill - Validate document consistency
