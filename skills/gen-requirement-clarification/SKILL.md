---
name: gen-requirement-clarification
...
- Bash 4.0+ (macOS/Linux/WSL) or PowerShell 5.1+ (Windows) for prerequisite check script
...
1. **Run prerequisites check script** to locate the feature specification:
    ...
    **macOS/Linux/WSL:**
    ...
    ```bash
    bash <SKILL_DIR>/scripts/check-prerequisites.sh --json --paths-only
    ```
...
4. **Recommendations**:
   ...
   - Suggest next step: `gen-architecture-design` skill, `gen-technical-detailed-design` skill, or another review
...
3. Suggest: "Spec is ready for next phase: architecture-design or technical-detailed-design skill"
...
- `check-prerequisites.sh` - Bash script to locate spec file (macOS/Linux/WSL)
- `check-prerequisites.ps1` - PowerShell script to locate spec file (Windows/cross-platform)
...
3. **Use architecture-design skill**: Build architectural design (new products)
4. **Use technical-detailed-design skill**: Create implementation plan (existing products)
