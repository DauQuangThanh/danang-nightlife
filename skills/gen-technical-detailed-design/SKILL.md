---
name: gen-technical-detailed-design
...
# General Technical Detailed Design
...
- Use `gen-coding-plan` skill to create task breakdown (tasks.md)
- Use `gen-code-implementation` skill to execute implementation
- Use `gen-code-review` skill after implementation for quality validation
...
1. **Run setup script** to create directory structure and copy templates:
   ...
   ```bash
   bash <SKILL_DIR>/scripts/setup-design.sh --json              # macOS/Linux/WSL
   .\<SKILL_DIR>\scripts\setup-design.ps1 -Json                 # Windows PowerShell
   ```
...
- **Automate Setup**: Use the provided setup script for consistent setup
