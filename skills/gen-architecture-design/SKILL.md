...
```bash
bash <SKILL_DIR>/scripts/setup-architect.sh                    # macOS/Linux/WSL
.\<SKILL_DIR>\scripts\setup-architect.ps1                      # Windows PowerShell
```
...
```bash
bash <SKILL_DIR>/scripts/update-agent-context.sh [agent-type]              # macOS/Linux/WSL
.\<SKILL_DIR>\scripts\update-agent-context.ps1 [-AgentType agent-type]     # Windows PowerShell
```
...
- If review passes, proceed to create feature specifications using `gen-requirement-development` skill
- Use `gen-technical-detailed-design` skill for individual feature implementation planning
