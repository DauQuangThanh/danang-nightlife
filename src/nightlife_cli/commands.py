"""CLI commands for Nightlife CLI."""
...
def init(
    ...
    """
    Initialize a new Nightlife project from the latest template.
    ...
    Examples:
        ...
        nightlife init --upgrade
        nightlife init --upgrade --ai claude
        nightlife init --here --upgrade

        # Use local templates for development
        nightlife init demo --local-templates --ai claude
        nightlife init demo --local-templates --template-path /path/to/danang-nightlife

        # With environment variables
        export RAINBOW_USE_LOCAL_TEMPLATES=1
        export RAINBOW_TEMPLATE_PATH=/path/to/danang-nightlife
        nightlife init demo --ai copilot
    """
        ...
        if project_path.exists() and not upgrade:
            ...
            if nightlife_installed:
                ...
                error_panel = Panel(
                    ...
                    "To upgrade the existing Nightlife installation, use:\n"
                    f"  [cyan]nightlife init {project_name} --upgrade[/cyan]\n\n"
                    "Or choose a different project name.",
                    title="[yellow]Nightlife Already Installed[/yellow]",
                    border_style="yellow",
                    padding=(1, 2)
                )
                console.print()
                ...
                # Directory exists but no Nightlife installation
                error_panel = Panel(
                    f"Directory '[cyan]{project_name}[/cyan]' already exists\n\n"
                    "Nightlife is not installed in this directory. You can:\n"
                    "  • Use [cyan]--force[/cyan] to merge Nightlife files into the existing directory\n"
                    f"  • Use [cyan]nightlife init . --force[/cyan] from within the directory\n"
                ...
                if not force and sys.stdin.isatty():
                    response = typer.confirm("\nDo you want to proceed and merge Nightlife files into this directory?")
                    if response:
    ...
    setup_lines = [
        f"[cyan]Nightlife Project {'Upgrade' if is_upgrade_mode else 'Setup'}[/cyan]",
        "",
    ...
    tracker = StepTracker("Upgrade Nightlife Project" if is_upgrade_mode else "Initialize Nightlife Project")
...
def check():
    ...
    console.print("\n[bold green]Nightlife CLI is ready to use![/bold green]")
...
def version():
    ...
    cli_version = "unknown"
    try:
        cli_version = importlib.metadata.version("nightlife-cli")
    except Exception:
    ...
    panel = Panel(
        info_table,
        title="[bold cyan]Nightlife CLI Information[/bold cyan]",
        border_style="cyan",
        padding=(1, 2)
    )
