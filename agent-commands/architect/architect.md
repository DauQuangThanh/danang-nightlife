...
1. **Design deployment architecture**:
   ...
   - Nightlife infrastructure components (compute, storage, networking)
...
2. **Design CI/CD pipeline**:
   ...
   - Nightlife deployment strategy (blue/green, canary, rolling)
...
/nightlife.architect Focus on scalability and multi-region deployment

# Generate architecture with constraints
/nightlife.architect Budget-conscious architecture with AWS services only
```

## Notes

- This command creates **product-level architecture**, not feature-level plans
- Run this AFTER creating feature specs (`/nightlife.specify`) and ground-rules
- Run this BEFORE creating feature implementation plans (`/nightlife.design`)
