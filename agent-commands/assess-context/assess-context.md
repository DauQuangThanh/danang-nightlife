...
handoffs:
  - label: Set Project Principles
    agent: nightlife.set-ground-rules
  ...
  - label: Get Architecture Review
    agent: nightlife.architect
    prompt: Review architecture assessment and provide recommendations
    send: true
scripts:
   py: python scripts/setup-assess-context.py --json
agent_scripts:
...
/nightlife.assess-context Analyze our e-commerce platform to understand architecture and patterns
# Then follow up with:
# /nightlife.set-ground-rules (create principles based on assessment)
# /nightlife.specify (add new features)

# Assess full stack Node.js/React application
/nightlife.assess-context Analyze our e-commerce platform before adding new payment gateway

# Assess Python Django application
/nightlife.assess-context Review our Django REST API codebase structure and conventions

# Assess with specific focus
/nightlife.assess-context Focus on testing patterns and API integration conventions for our microservices

# Quick assessment for small projects
/nightlife.assess-context Quick assessment of project structure and conventions
```

## Notes

- Run this ONCE per project when first adding Nightlife to existing brownfield codebase
...
- **Recommended workflow**: `/nightlife.assess-context` → `/nightlife.set-ground-rules` → `/nightlife.specify`
- Follow up with `/nightlife.set-ground-rules` to create project principles based on assessment
- Then use `/nightlife.specify` to create feature specifications
